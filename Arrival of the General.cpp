...
void solve(){
   ...
   cin>>n;
   vector<ll>vx(n);
   ll tmax=0,tmin=1e9,posmax,posmin;
   fai(i,n){
   	cin>>vx[i];
   	if(tmax < vx[i]){tmax=vx[i];posmax=i;}
    if(tmin>=vx[i]){tmin=vx[i];posmin=i;}
   }
  // cout<<tmax<<posmax<<" "<<tmin<<posmin<<nl;
   if(posmax<posmin) cout<<posmax+(n-posmin-1)<<nl;
   else cout<<posmax+(n-posmin-2)<<nl;
}
