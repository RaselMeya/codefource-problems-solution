...
void solve(){
	 ...
	 cin>>n;
	  ll arr[n];
	  fai(i,n)cin>>arr[i];
	  sort(arr,arr+n);                             // 1 2 3 4 5 6
	   ll cnt=0;
	   vector<ll> vx;                         // 1 2 3         //  4 5  6  // 4 1 5 2 6 3
	  for(ll i= n/2 ,k=0; i< n ; i++){
	  	vx.pb(arr[i]);
	  	if(k< n/2)vx.pb(arr[k++]);
	  }
	  for(ll i=1; i<vx.sz()-1; i++){
	  	if(vx[i]< vx[i-1] and vx[i]< vx[i+1])cnt++;
	  }
	  cout<<cnt<<nl;
